...
void read_cb(struct bufferevent* bev, void* arg)
{
    ...
    bufferevent_write(bev, ptr, strlen(ptr));//写入时会自动发送，默认函数调用后在调用w_cb主要是为了进行通知
                ...
                int len, void* ptr)
{
    ...
    //先接收数据，在发送数据
    //将fd封装为bufferevent事件中
    struct  bufferevent* bev = NULL;
    bev = bufferevent_socket_new(base, fd, BEV_OPT_CLOSE_ON_FREE);//ON_FREE 不能释放事件而是释放事假所对应的文件描述符资源
